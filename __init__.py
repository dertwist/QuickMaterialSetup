bl_info = {
    ...
    "version": (1, 3),
    "support": "COMMUNITY",
    "wiki_url": 'https://github.com/dertwist/QuickMaterialSetup',
}
...
class ExampleAddonPreferences(bpy.types.AddonPreferences):
    ...
    default="UnrealEnginePreset_ORM UnrealEnginePreset_RMA Source2Preset", 
    description='"T_"YourTexture'
    )

    sufix_bc: bpy.props.StringProperty(
    default="_BC  _BaseColor _Albedo _color _Color",
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.prop(self, "sufix_bc", text="Unreal Engine Sufix for color textures")
        row = layout.row()
        row.prop(self, "sufix_pack", text="Unreal Engine Sufix for packed textures")
        row = layout.row()
        row.prop(self, "sufix_n", text="Unreal Engine Sufix for normal textures")
        row.separator()

class FastMaterialsPreferences(bpy.types.PropertyGroup):
    texture_prefixes: StringProperty(
        name='Base Color',
        default='BC BaseColor Albedo Color color',
        description='YourTexture"_BC"')
...
class SetupMaterialOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        print (tex_basecolor_suf)

        #Source2

        source2_tex_basecolor_suf = ['_Color', '_Albedo']
        source2_tex_rough_suf = ['_Rough']
        source2_tex_metal_suf =['_Metal']
        ...
        if preset_name == "Source2Preset":
           for i in range(len(exstensions)):
                if exstensions[i] in texture_basecolor_path:
                    ...
                    pass
                print (texture_basecolor_path)
                for sufix in source2_tex_basecolor_suf:
                    try:
                        ...
                        obj.material_slots[0].material.node_tree.nodes["Color"].image = new_img
                    except:
                        print(os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        pass             
                for sufix in source2_tex_rough_suf:
                    try:
                        new_img = bpy.data.images.load(filepath = os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        obj.material_slots[0].material.node_tree.nodes["Rough"].image = new_img
                        try:
                            obj.material_slots[0].material.node_tree.nodes["Rough"].image.colorspace_settings.name='Non-Color'
                        except:
                            obj.material_slots[0].material.node_tree.nodes["Rough"].image.colorspace_settings.name='Utility - Raw'
                    except:
                        # print(os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        pass     
                for sufix in tex_normal_suf:
                    try:
                        ...
                        # print(os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        pass   
                for sufix in source2_tex_metal_suf:
                    try:
                        new_img = bpy.data.images.load(filepath = os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        obj.material_slots[0].material.node_tree.nodes["Metal"].image = new_img
                        try:
                            obj.material_slots[0].material.node_tree.nodes["Metal"].image.colorspace_settings.name='Non-Color'
                        except:
                            obj.material_slots[0].material.node_tree.nodes["Metal"].image.colorspace_settings.name='Utility - Raw'
                    except:
                        # print(os.path.dirname(texture_basecolor_path) + prefix_fix + texture_basecolor_name + sufix + extension)
                        pass  
        else:
...
class MT_Preset_Source2(bpy.types.Operator):
    bl_idname = "object.preset_source2"
    bl_label = "Source 2 "
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        global preset_name
...
class MT_PresetsMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator('object.preset_source2')
